...
namespace ImGui
{
    ...
    static const WaveformPalette &ActivePalette_()
    {
        static constexpr WaveformPalette kDark = {
        ...
        static constexpr WaveformPalette kLight = {
    ...
    static void ClampView(LoopWaveformState &st, u32 totalSamples)
    {
        ...
        if (viewStart < 0)
            viewStart = 0;
        if (viewEnd - viewStart < 16)
            viewEnd = std::min(total, viewStart + 16);
                              ...
                              const LoopWaveformState &st, u32 sampleRate)
    {
        ...
        for (double t = first; t <= endMs; t += stepMs)
        {
            ...
            draw->AddLine(ImVec2(origin.x + x, origin.y), ImVec2(origin.x + x, origin.y + 8), ActivePalette_().edgeSoft);

            if (stepMs < 1.0)
                snprintf(label, sizeof(label), "%.1fms", t);
            else if (t >= 1000.0)
                snprintf(label, sizeof(label), "%.*fs", std::fmod(t, 1000.0) != 0 ? 2 : 0, t / 1000.0);
            else
                snprintf(label, sizeof(label), "%.0fms", t);

            draw->AddText(ImVec2(origin.x + x + 3, origin.y + 1), ActivePalette_().inkFaint, label);
        }
    }

    static void DrawHandle(ImDrawList *draw, ImVec2 origin, float x, float H, bool isStart, bool focused)
    {
        draw->AddLine(ImVec2(origin.x + x, origin.y), ImVec2(origin.x + x, origin.y + H), ActivePalette_().loop, focused ? 2.0f : 1.0f);
        const float tri = 6.0f;
        if (isStart)
            draw->AddTriangleFilled(ImVec2(origin.x + x, origin.y), ImVec2(origin.x + x + tri, origin.y),
                                    ImVec2(origin.x + x, origin.y + tri), ActivePalette_().loop);
        else
            draw->AddTriangleFilled(ImVec2(origin.x + x, origin.y), ImVec2(origin.x + x - tri, origin.y),
                                    ImVec2(origin.x + x, origin.y + tri), ActivePalette_().loop);
    }
                             ...
                             u32 loopStart, u32 loopEnd, float playheadSample, bool showLoop)
    {
        ...
        draw->AddRectFilled(origin, ImVec2(origin.x + W, origin.y + H), ActivePalette_().bg);
        ...
        if (showLoop)
        {
            ...
            draw->AddRectFilled(origin, ImVec2(origin.x + cS, origin.y + H), ActivePalette_().introGlow);
            draw->AddRectFilled(ImVec2(origin.x + cS, origin.y), ImVec2(origin.x + cE, origin.y + H), ActivePalette_().loopGlow);
            ...
            if (xfSamp > 0)
            {
                ...
                draw->AddRectFilled(ImVec2(origin.x + std::max(0.0f, xS), origin.y), ImVec2(origin.x + std::max(0.0f, xS) + xfPx, origin.y + H), ActivePalette_().xfadeFill);
                draw->AddRectFilled(ImVec2(origin.x + std::max(0.0f, xE - xfPx), origin.y), ImVec2(origin.x + std::max(0.0f, xE), origin.y + H), ActivePalette_().xfadeFill);
            }
        }

        auto yOf = [mid, H](float v)
        { return mid - v * (H * 0.5f) * 0.92f; };
        ...
        if (spp <= 1.0f)
        {
            ...
            for (u32 s = s0; s < s1; s++)
            {
                ...
                const ImU32 col = inLoop ? ActivePalette_().waveLit : ActivePalette_().wave;
        ...
        if (playheadSample >= 0.0f && playheadSample + spp2 >= st.viewStart && playheadSample <= st.viewEnd + spp2)
        {
            ...
            draw->AddLine(ImVec2(origin.x + px, origin.y), ImVec2(origin.x + px, origin.y + H), ActivePalette_().good, 1.5f);
            draw->AddTriangleFilled(ImVec2(origin.x + px - 4, origin.y + H), ImVec2(origin.x + px + 4, origin.y + H), ImVec2(origin.x + px, origin.y + H - 6), ActivePalette_().good);
        }
                            ...
                            ImVec2 size)
    {
        ...
        if (!state.viewInitialized && n > 0)
        {
            ...
            state.viewEndAnim = static_cast<double>(n);
            state.viewStartTarget = 0.0;
            state.viewEndTarget = static_cast<double>(n);
        ...
        if (n > 0)
        {
            ...
            if (vs < 0)
                vs = 0;
            if (ve > static_cast<s64>(n))
                ve = n;
            if (ve <= vs)
                ve = std::min<s64>(n, vs + 1);
            if (vs >= ve)
                vs = ve > 0 ? ve - 1 : 0;
        ...
        if (n > 0)
            ...
            draw->AddRectFilled(origin, ImVec2(origin.x + size.x, origin.y + size.y), ActivePalette_().bg);

        if (n > 0 && readoutText && readoutText[0])
            draw->AddText(ImVec2(origin.x + 9.0f, origin.y + 6.0f), ActivePalette_().inkFaint, readoutText);
        ...
        if (n > 0)
        {
            ...
            draw->AddRectFilled(trackOrigin, ImVec2(trackOrigin.x + size.x, trackOrigin.y + trackH), ActivePalette_().edge);
            ...
            const ImU32 thumbCol = (state.dragging == LoopWaveformState::Dragging::Scrollbar) ? ActivePalette_().waveLit : ActivePalette_().loop;
...

        {
            ...
            if (z < 1.05f)
                ...
                snprintf(zoomLabel, sizeof(zoomLabel), z < 10.0f ? "%.1f\xc3\x97" : "%.0f\xc3\x97", z);

            if (SmallButton("-"))
                ZoomAt(state, (state.viewStart + state.viewEnd) / 2.0, 1.4, n);
            SameLine();
            Text("zoom %s", zoomLabel);
            SameLine();
            if (SmallButton("+"))
                ZoomAt(state, (state.viewStart + state.viewEnd) / 2.0, 1.0 / 1.4, n);
            SameLine();
            if (Button("Fit loop"))
                ZoomToLoop(state, loopStart, loopEnd, n);
            SameLine();
            if (Button("Fit all"))
                ResetView(state, n);
        }
    ...
    void LoopSeamPreview(const std::vector<float> &mono, u32 loopStart, u32 loopEnd, u32 period, u32 sampleRate, float crossfadeMs, bool equalPowerCurve, ImVec2 size)
    {
        ...
        ImVec2 origin = GetCursorScreenPos();
        ImDrawList *draw = GetWindowDrawList();
        draw->AddRectFilled(origin, ImVec2(origin.x + size.x, origin.y + size.y), ActivePalette_().seamBg);
        Dummy(size);

        if (n == 0 || loopEnd <= loopStart)
            return;
        ...
        draw->AddLine(ImVec2(origin.x, mid), ImVec2(origin.x + size.x, mid), ActivePalette_().edge);
        ...
        plot([&](u32 idx)
             { return mono[ClampIdx(static_cast<s64>(loopEnd) - static_cast<s64>(half) + idx, n)]; }, ActivePalette_().waveLit);
        plot([&](u32 idx)
             { return mono[ClampIdx(static_cast<s64>(loopStart) - static_cast<s64>(half) + idx, n)]; }, ActivePalette_().loop);
        ...
        for (u32 i = 0; i + 1 < view; i++)
        {
            ...
            draw->AddLine(ImVec2(origin.x + x0, y0), ImVec2(origin.x + x1, y1), ActivePalette_().good, 1.6f);
        }
